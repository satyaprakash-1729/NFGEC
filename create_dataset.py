...
def create_dataset(corpus_path,label2id,word2id,feature2id):
    ...
    num_of_samples = len(json.load(open(corpus_path)))
    storage = []
    ...
    with open(corpus_path) as f:
        ...
        for i in range(size):
            for j in range(len(data_all[i]["mentions"])):
                (start,end,words,labels,features) = data_all[i]["mentions"]["start"],
                    data_all[i]["mentions"]["end"], data_all[i]["tokens"], data_all[i]["mentions"][j]["labels"],
                    data_all[i]["mentions"][j]["features"]
                length = len(words)
